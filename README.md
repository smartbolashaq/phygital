...
`models/assembly.glb` — нижняя часть + крыша в одном файле

`assembly.glb` собран из двух исходников (model3d2 + model3d3) скриптом
merge_glb.py. Это важно: если грузить половинки в два отдельных
<model-viewer>, каждый вписывает свою модель в свой контейнер, и маленькая
крыша раздувается до размера всей машины. В одном файле пропорции честные.

Крыша лежит в узле с именем `RoofGroup` — по нему её двигает скролл.
...
до 8 бит, из-за чего на глянцевых плоскостях (лобовое стекло, бампер)
появляются пятна и полосы.
